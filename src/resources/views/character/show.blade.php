...
@section('page_header', 'Tax Details: ' . $character->name)

@section('full')
<div class="row">
    <div class="col-md-12">
        <a href="{{ route('alliancetax.dashboard') }}" class="btn btn-default margin-bottom">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">{{ number_format($tax->applicable_tax_rate ?? $tax->tax_rate, 2) }}%</td>
                            <td class="text-right">{{ number_format($tax->tax_amount, 2) }} ISK</td>
                            <td>
                                @if($tax->status === 'paid' || $tax->is_paid)
                                    <span class="label label-success">Paid</span>
                                @elseif($tax->status === 'pending')
                                    <span class="label label-warning">Pending</span>
                                @else
                                    <span class="label label-danger">{{ ucfirst($tax->status ?? 'Unknown') }}</span>
                                @endif
                            </td>
                            <td>
                                @can('alliancetax.admin')
                                <form method="POST" action="{{ route('alliancetax.admin.calculations.destroy', $tax->id) }}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-xs btn-danger" onclick="return confirm('DELETE this tax record and its associated invoice (if any)?')">
